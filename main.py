...
def main():
    ...
    while running:
        for event in pg.event.get():
            if event.type == pg.QUIT:
                ...
                if event.key == pg.K_SPACE and not extinct:
                    paused = not paused
                elif event.key == pg.K_ESCAPE and extinct:
                    running = False
        ...
        action_text = '(ESC to Exit)' if extinct else ('(SPACE to Resume)' if paused else '(SPACE to Pause)')
        stats = [
        ...
        if paused or extinct:
            ...
            # Draw semi-transparent background for overlay text for contrast
            bg_rect = overlay_rect.inflate(40, 20)
            overlay_bg = pg.Surface((bg_rect.width, bg_rect.height), pg.SRCALPHA)
            overlay_bg.fill((0, 0, 0, 180))
            screen.blit(overlay_bg, bg_rect)
