...
def main():
    ...
    while running:
        ...
        toggle_text = "" if extinct else ("(SPACE to Resume)" if paused else "(SPACE to Pause)")
        stats = [
        ...
        if extinct:
            ...
            overlay = large_font.render("EXTINCTION", True, (255, 50, 50))
            overlay_rect = overlay.get_rect(center=(width * tile_size // 2, height * tile_size // 2 - 20))
            screen.blit(overlay, overlay_rect)

            sub_text = font.render("Press ESC to Exit", True, (255, 255, 255))
